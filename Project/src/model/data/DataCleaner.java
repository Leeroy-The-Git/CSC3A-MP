...
/**
 * Class dedicated to the cleaning of data
...
//	private static final String COMICS_FINAL_NAME = "comics.txt";
	...
	public static void cleanData() throws FileNotFoundException, IOException {
		try { 
			...
			File file = new File(path + EDGES_FINAL_NAME);
			if (!file.exists())
				file.createNewFile();
...
//			file = new File(path + FINAL_DIR + COMICS_FINAL_NAME);
//			if (!file.exists())
//				file.createNewFile();
...
//			pw.close();
//			
//			file = new File(path + FINAL_DIR + HEROES_FINAL_NAME);
//			if (!file.exists())
//				file.createNewFile();
